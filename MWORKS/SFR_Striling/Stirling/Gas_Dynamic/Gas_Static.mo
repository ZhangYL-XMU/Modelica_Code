within SFR_Striling.Stirling.Gas_Dynamic;
model Gas_Static
  extends SFR_Striling.Stirling.Gas_Dynamic.Gas_Partial;
  // 系统初始参数----------------------------------------------------------------
  // 系统内的起始气压
  ...
  // 气体工质的总物质的量
   ...
   // 通道内工质的气压
   .Real P_l(start = Po);
   // 环隙通道内工质的流速（原 Gas_Dynamic_new 中误注释为“膨胀活塞的运动速度”）
   .Real c;
   // 压缩气缸内工质的质量流
   .Real mp2   ;
   // 膨胀气缸内工质的质量流
...
equation
  ...
  // 根据工质的流动方向判断质量流走向（noEvent 消除过零颤振）
  if noEvent(c<0) then
    mp1 = c*A_an*ro_l*1;
    mp2 = c*A_an*ro2*1;
  else
