within SFR_Striling.Structure.Closed;
model v2
  "无PID自稳验证变体"
  parameter Modelica.Units.SI.Power Q0_start = 40e6 "点堆初始裂变功率（自稳=40e6；扰动=44e6）";
  annotation(__MWORKS(version="26.6.0"),Diagram(coordinateSystem(extent={{-100,-100},{100,100}},
grid={2,2})));
  Modelica.Mechanics.Rotational.Components.Fixed fixed_ground annotation(Placement(transformation(origin={219.239,-200},
extent={{-10,-10},{10,10}})));
...
extent={{10,-10},{-10,10}})));
  Modelica.Mechanics.Rotational.Components.Damper damper(d=10.6) "发电负载（同步 SecondLoop：转速→1500rpm）" annotation(Placement(transformation(origin={237.239,-176},
extent={{10,-10},{-10,10}})));
  Modelica.Mechanics.Rotational.Components.Inertia inertia(J=2, phi(start=0, fixed=true), w(start=0, fixed=true) "初值0（同 SecondLoop；稳态自然转向负向）") 
    annotation(Placement(transformation(origin={272,-176},
extent={{-10,-10},{10,10}})));
...
extent={{10,10},{-10,-10}},
rotation=90)));
  TRANSFORM.Fluid.Machines.Pump_SimpleMassFlow pump1(m_flow_nominal=280.6, redeclare package Medium = Media.Sodium.ConstantPropertyLiquidSodium) 
...
extent={{10,-10},{-10,10}})));
  Modelica.Blocks.Sources.RealExpression realExpression1(y=innerCore.T[3] * 0.2237 + outerCore.T[3] * 0.7763) 
    annotation (Placement(transformation(origin={-298,-26.5},
extent={{-10,-10},{10,10}})));
  Modelica.Blocks.Sources.RealExpression realExpression_fuel(y=0.2237 * innerCore.T_wall[3] + 0.7763 * outerCore.T_wall[3]) "燃料反馈: 份额加权管道壁温(内0.2237/外0.7763, 轴向T_wall[3])" 
    annotation (Placement(transformation(origin={-314,8},
extent={{-10,-10},{10,10}})));
...
extent={{-10,-10},{10,10}})));
  Modelica.Blocks.Sources.Constant constCR(k=0) "无PID：控制棒反应性恒为0（自稳验证）" 
    annotation (Placement(transformation(origin={-320,68.5},
extent={{-10,-10},{10,10}})));
  TYThermoFluidSys.Blocks.Constant const1(k=0) 
    annotation (Placement(transformation(origin={-320,35.5},
extent={{-10,-10},{10,10}})));
  Nuclear.PointKinetics pointKinetics(Teffref_fuel=863.2 "无PID自稳试验: 燃料参考=额定份额加权管道壁温(PrimaryLoop_noPID参数)" , Teffref_coolant(displayUnit="K")=768.29 "无PID自稳试验: 冷却剂参考(PrimaryLoop_noPID参数)参考=额定堆芯平均温度(440+550)/2=495℃=768.15K(尹凯论文口径); 初始冷却剂反馈=0, 功率不跌落", Q_fission(start=Q0_start)) 
    annotation (Placement(transformation(origin={-244,35.5},
extent={{-18,-20.5},{18,20.5}})));
  Fluid.Vessels.MixingVolume volume1(redeclare package Medium = Media.Sodium.ConstantPropertyLiquidSodium, nPorts_a=1, nPorts_b=3, V=10, h_start=519165 "下腔室440℃") 
    annotation (Placement(transformation(origin={-52,-84.5},
extent={{-10,-10},{10,10}},
rotation=90)));
  Fluid.Vessels.MixingVolume volume(nPorts_a=3, nPorts_b=1, V=10, h_start=656775 "上腔室550℃", redeclare package Medium = Media.Sodium.ConstantPropertyLiquidSodium) 
    annotation (Placement(transformation(origin={-52,111.5},
extent={{-10,-10},{10,10}},
rotation=90)));
  Fluid.Pipes.pipe pipe_static2(N=5, redeclare package Medium = Media.Sodium.ConstantPropertyLiquidSodium, L_total=10, Dh=0.30, initFromEnthalpy=true, h_start=519165 "冷段440℃", wallHeatTransfer=false, momentumDynamics=Modelica.Fluid.Types.Dynamics.SteadyState) 
    annotation (Placement(transformation(origin={46,-90.5},
extent={{10,-10},{-10,10}})));
  Fluid.Pipes.pipe pipe_static1(N=5, redeclare package Medium = Media.Sodium.ConstantPropertyLiquidSodium, L_total=10, Dh=0.30, initFromEnthalpy=true, h_start=656775 "热段550℃", wallHeatTransfer=false, momentumDynamics=Modelica.Fluid.Types.Dynamics.SteadyState) 
    annotation (Placement(transformation(origin={65,133.5},
extent={{-10,-10},{10,10}})));
  Modelica.Blocks.Sources.RealExpression realExpression3[4] (y=pointKinetics.Q_total * 0.7763 / 4 "外堆芯 77.63%(PrimaryLoop_noPID参数)") 
    annotation (Placement(transformation(origin={-106,35.5},
extent={{-10,-10},{10,10}})));
  Fluid.Vessels.ExpansionTank expansionTank(redeclare package Medium = Media.Sodium.ConstantPropertyLiquidSodium, A=1, V0=0.001, level_start=1, h_start=656775 "热段550℃", p_start=100000.0) 
    annotation (Placement(transformation(origin={96,195.5},
extent={{-10,-10},{10,10}})));
  TYThermoFluidSys.Sensors.SensorT TSensor2(redeclare package Medium = Media.Sodium.ConstantPropertyLiquidSodium) 
    annotation (Placement(transformation(origin={-27,153.5},
extent={{-10,-10},{10,10}})));
  Modelica.Blocks.Sources.RealExpression realExpression4[4] (y=pointKinetics.Q_total * 0.2237 / 4 "内堆芯 22.37%(PrimaryLoop_noPID参数)") 
...
points={{-15.239,-24.34},{-8,-24.34},{-8,14.8}}));
  ...
  annotation(Line(origin={347.239,-186},
points={{-120,10},{-128,10},{-128,-14}},
color={0,0,0}));
  connect(damper.flange_a, inertia.flange_a) 
  annotation(Line(origin={256.25,-190.96},
...
points={{-236,-0.51625},{-200,-0.51625},{-200,4.25},{-190.755,4.25}},
color={0,0,127}));
  connect(realExpression1.y, pointKinetics.Teff_coolant) 
  annotation(Line(origin={-73,36.01625},
points={{-214,-62.51625},{-200,-62.51625},{-200,-16.045},{-190.755,-16.045}},
color={0,0,127}));
  connect(const3.y, pointKinetics.Q_in) 
  annotation(Line(origin={-73,36.01625},
points={{-182,65.48375},{-171.315,65.48375},{-171.315,22.085}},
color={0,0,127}));
  connect(realExpression_fuel.y, pointKinetics.Teff_fuel) 
  annotation(Line(origin={-73,36.01625},
points={{-230,-28.01625},{-209,-28.01625},{-209,-5.8975},{-190.755,-5.8975}},
color={0,0,127}));
...
points={{-27,-5.5},{14.49,-5.5},{14.49,-24.0988},{18.245,-24.0988}},
color={0,0,127}));
  end v2;