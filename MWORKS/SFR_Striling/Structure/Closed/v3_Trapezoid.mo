...
extent={{-10,-10},{10,10}})));
  Modelica.Blocks.Continuous.PID PID(k=0.5, Ti=100, Td=0, initType=Modelica.Blocks.Types.Init.NoInit) "NoInit：避免与 PointKinetics.Q_fission(fixed=true) 初始条件冗余 6142（2026-09-01；与 PrimaryLoop 一致）" 
    annotation (Placement(transformation(origin={-435.991,156},
extent={{-10,-10},{10,10}})));
  Modelica.Blocks.Math.Add PID_error(k1=1, k2=-1) "误差 e=u_s-u_m (归一化后)" 
    annotation (Placement(transformation(origin={-480,150},
extent={{-10,-10},{10,10}})));
  Modelica.Blocks.Nonlinear.Limiter limPID(uMin=-0.00171 * 2, uMax=0.00171 * 2) 
    annotation (Placement(transformation(origin={-391.982,155.9987},
extent={{-10,-10},{10,10}})));
  Modelica.Blocks.Math.Gain gain_uS(k=1 / 40e6) "u_s 归一化(ramp W→无量纲)" 
    annotation (Placement(transformation(origin={-518,156},
extent={{-10,-10},{10,10}})));
  Modelica.Blocks.Math.Gain gain_uM(k=1 / 40e6) "u_m 归一化(Q_total W→无量纲)" 
    annotation (Placement(transformation(origin={-474,111.599},
extent={{-10,-10},{10,10}})));
  Modelica.Blocks.Sources.Trapezoid trapezoid(offset=40e6,startTime=1000,amplitude=4e6,rising=120,width=300,falling=120,period=720) 
    annotation (Placement(transformation(origin={-562,156},
extent={{-10,-10},{10,10}})));
  equation
...
color={0,0,127}));
  ...
  annotation(Line(origin={-118.982,142.115},
points={{-164.903,-17.3857},{-135.018,-17.3857},{-135.018,-68.115},{-371.018,-68.115},{-371.018,-30.5163},{-367.018,-30.5163}},
color={0,0,127}));
  connect(gain_uM.y, PID_error.u2) 
  annotation(Line(origin={-423.982,135.5987},
points={{-39.018,-24},{12,-24},{12,-8},{-72.018,-8},{-72.018,8.4013},{-68.018,8.4013}},
color={0,0,127}));
  connect(gain_uS.y, PID_error.u1) 
  annotation(Line(origin={-433.982,157.5987},
points={{-73.018,-1.5987},{-58.018,-1.5987}},
color={0,0,127}));
  connect(PID_error.y, PID.u) 
  annotation(Line(origin={-425.982,147.5987},
points={{-43.018,2.4013},{-26.009,2.4013},{-26.009,8.4013},{-22.009,8.4013}},
color={0,0,127}));
  connect(PID.y, limPID.u) 
  annotation(Line(origin={-405.982,155.5987},
points={{-19.009,0.4013},{2,0.4}},
color={0,0,127}));
  connect(limPID.y, pointKinetics.Reactivity_CR) 
  annotation(Line(origin={-352,140},
  points={{-28.982,15.9987},{12,15.9987},{12,-15.0657},{28.245,-15.0657}},
  color={0,0,127}));
  connect(gain_uS.u, trapezoid.y) 
  annotation(Line(origin={-540,156},
  points={{10,0},{-11,0}},
  color={0,0,127}));
  end v3_Trapezoid;