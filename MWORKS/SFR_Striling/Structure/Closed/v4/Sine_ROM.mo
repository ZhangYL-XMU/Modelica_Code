...
rotation=90)));
  Modelica.Blocks.Sources.RealExpression realExpression2(y=(Engine_pipe.T[1] + Engine_pipe.T[2] + Engine_pipe.T[3] + Engine_pipe.T[4]) / 4) 
    annotation (Placement(transformation(origin={208,-103.983},
extent={{-10,-10},{10,10}})));
...
rotation=90)));
  Modelica.Blocks.Math.Gain gain_uM(k=1 / 40e6) "u_m 归一化(Q_total W→无量纲)" 
    annotation (Placement(transformation(origin={-472.009,53.599},
extent={{-10,-10},{10,10}})));
  Modelica.Blocks.Math.Gain gain_uS(k=1 / 40e6) "u_s 归一化(ramp W→无量纲)" 
    annotation (Placement(transformation(origin={-516.009,98},
extent={{-10,-10},{10,10}})));
  Modelica.Blocks.Math.Add PID_error(k1=1, k2=-1) "误差 e=u_s-u_m (归一化后)" 
    annotation (Placement(transformation(origin={-478.009,92},
extent={{-10,-10},{10,10}})));
  Modelica.Blocks.Continuous.PID PID(k=0.5, Ti=100, Td=0, initType=Modelica.Blocks.Types.Init.NoInit) "NoInit：避免与 PointKinetics.Q_fission(fixed=true) 初始条件冗余 6142（2026-09-01；与 PrimaryLoop 一致）" 
    annotation (Placement(transformation(origin={-434,98},
extent={{-10,-10},{10,10}})));
  Modelica.Blocks.Nonlinear.Limiter limPID(uMin=-(0.00171 * 2 + 0.0779), uMax=0.00171 * 2 + 0.0779) 
    annotation (Placement(transformation(origin={-389.991,98},
extent={{-10,-10},{10,10}})));
  Modelica.Blocks.Sources.Sine sine(offset=40e6, startTime=800, f=0.0005, amplitude=4e6) 
    annotation (Placement(transformation(origin={-566,98},
extent={{-10,-10},{10,10}})));
  v4.ROM.ROM_Basis rOM_Basis 
    annotation (Placement(transformation(origin={264,-105.5},
extent={{-21,-20.5},{21,20.5}})));
  equation
...
color={0,127,255}));
  ...
  annotation(Line(origin={-116.991,84.115},
points={{-166.147,-18.6832},{-135.018,-18.6832},{-135.018,-74.115},{-371.018,-74.115},{-371.018,-30.516},{-367.018,-30.516}},
color={0,0,127}));
  connect(gain_uM.y, PID_error.u2) 
  annotation(Line(origin={-421.991,77.5987},
points={{-39.018,-23.9997},{12,-23.9997},{12,-8},{-72.018,-8},{-72.018,8.4013},{-68.018,8.4013}},
color={0,0,127}));
  connect(gain_uS.y, PID_error.u1) 
  annotation(Line(origin={-431.991,99.5987},
points={{-73.018,-1.5987},{-58.018,-1.5987}},
color={0,0,127}));
  connect(PID_error.y, PID.u) 
  annotation(Line(origin={-423.991,89.5987},
points={{-43.018,2.4013},{-26.009,2.4013},{-26.009,8.4013},{-22.009,8.4013}},
color={0,0,127}));
  connect(PID.y, limPID.u) 
  annotation(Line(origin={-412,98},
points={{-11,0},{10.009,0}},
color={0,0,127}));
  connect(limPID.y, pointKinetics.Reactivity_CR) 
  annotation(Line(origin={-351,82},
points={{-27.991,16},{15,16},{15,-16.3632},{27.992,-16.3632}},
color={0,0,127}));
  connect(gain_uS.u, sine.y) 
  annotation(Line(origin={-542,98},
  points={{13.991,0},{-13,0}},
  color={0,0,127}));
  connect(rOM_Basis.ROM_OUT[2], gain1.u) 
  annotation(Line(origin={297,-97},
  points={{-12,-6.98314},{11,-6.98314},{11,6.371}},
  color={0,0,127}));
  connect(realExpression2.y, rOM_Basis.ROM_IN[1]) 
  annotation(Line(origin={231,-104},
  points={{-12,0.016858},{12.42,0.016858}},
  color={0,0,127}));
  end Sine_ROM;