...
rotation=90)));
  ...
  SFR_Striling.Fluid.Vessels.MixingVolume volumeTube(redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, nPorts_a=1, nPorts_b=1, V=1, h_start=656775 "热段550℃") 
    annotation(Placement(transformation(origin={24,36},
extent={{-10,-10},{10,10}})));
  // ========== 膨胀箱定压支路（参照 IHTS5_AHX3：死端膨胀箱经固定流阻接冷段） ==========
  SFR_Striling.Fluid.Vessels.ExpansionTank expansionTank(redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, A=1, V0=0.001, level_start=1, h_start=506655, p_start=100000.0) 
    annotation(Placement(transformation(origin={38,158},
extent={{-10,-10},{10,10}})));
  SFR_Striling.Fluid.Vessels.SpecifiedResistance resistance_toExpTank(redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, R=0.1) 
    annotation(Placement(transformation(origin={64,134},
extent={{10,-10},{-10,10}})));
  // ========== 壳侧回路 ==========
  Modelica.Fluid.Sources.MassFlowSource_T boundary4(nPorts=1, redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, m_flow=280, T=823.15) 
    annotation(Placement(transformation(origin={-102,4},
extent={{-10,-10},{10,10}})));
  Modelica.Fluid.Sources.Boundary_pT boundary(nPorts=1, redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, T=713.15, p=100000.0) 
...
extent={{-10,-10},{10,10}})));
  TRANSFORM.Fluid.Machines.Pump_SimpleMassFlow pump2(m_flow_nominal=308.4, redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium) 
