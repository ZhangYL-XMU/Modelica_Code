...
model test_ExpansionTank "膨胀箱充液验证：der(level)=m_flow/(rho*A)，p=p_surface+rho*g*level"
  ...
  Modelica.Fluid.Sources.MassFlowSource_T boundary(nPorts=1, redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, m_flow=10, T=673.15) 
    annotation (Placement(transformation(origin={-70,0}, extent={{-10,-10},{10,10}})));
  SFR_Striling.Fluid.Vessels.ExpansionTank tank(redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, A=1, V0=0.001, level_start=1, h_start=469125, p_start=100000.0) 
