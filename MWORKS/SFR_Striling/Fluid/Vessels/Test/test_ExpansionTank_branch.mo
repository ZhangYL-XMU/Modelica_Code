...
model test_ExpansionTank_branch "膨胀箱支路验证：主管道三通接膨胀箱+阻力，稳态支路零流量、液位平衡"
  ...
  Modelica.Fluid.Sources.MassFlowSource_h boundary(nPorts=1, redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, m_flow=10, h=469125) 
    annotation (Placement(transformation(origin={-80,0}, extent={{-10,-10},{10,10}})));
  Modelica.Fluid.Fittings.TeeJunctionVolume tee(redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, V=0.01, p_start=108466.4, T_start=673.15) 
    annotation (Placement(transformation(origin={-20,0}, extent={{-10,-10},{10,10}})));
  SFR_Striling.Fluid.Vessels.SpecifiedResistance res(redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, R=0.1) 
    annotation (Placement(transformation(origin={0,-50}, extent={{-10,-10},{10,10}})));
  SFR_Striling.Fluid.Vessels.ExpansionTank tank(redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, A=1, V0=0.001, level_start=1, h_start=469125, p_start=108466.4, p_surface=100000.0) 
    annotation (Placement(transformation(origin={30,-50}, extent={{-10,-10},{10,10}})));
  Modelica.Fluid.Sources.Boundary_ph boundary1(nPorts=1, redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, p=108466.4, h=469125) 
    annotation (Placement(transformation(origin={80,0}, extent={{10,-10},{-10,10}})));
equation
