...
model test_SpecifiedResistance "固定水力阻力验证：m_flow*R = p_a - p_b（10 kg/s × R=100 → Δp=1000 Pa）"
  ...
  SFR_Striling.Fluid.Vessels.SpecifiedResistance res(redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, R=100) 
    annotation (Placement(transformation(origin={0,0}, extent={{-10,-10},{10,10}})));
  Modelica.Fluid.Sources.Boundary_ph boundary1(nPorts=1, redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, p=1e5, h=469125) 
    annotation (Placement(transformation(origin={80,0}, extent={{10,-10},{-10,10}})));
equation
