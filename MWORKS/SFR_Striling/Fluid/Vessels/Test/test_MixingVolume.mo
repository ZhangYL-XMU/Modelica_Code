...
model test_MixingVolume "混合体积自写实现验证：单进单出守恒 + 双进单出混合"
  ...
  MixingVolume volume(redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, nPorts_a=1, nPorts_b=1, p_start=1e5, T_start=823.15, V=10) 
  ...
  Modelica.Fluid.Sources.Boundary_ph boundary1(nPorts=1, redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, p=1e5, h=656775) 
    annotation (Placement(transformation(origin={10,40}, extent={{10,-10},{-10,10}})));

  // ---------- 场景二：双进单出（验证混合温度：两股 5 kg/s，823.15 K 与 903.09 K 混合） ----------
  MixingVolume volume2(redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, nPorts_a=2, nPorts_b=1, p_start=1e5, T_start=863.12, V=10) 
    annotation (Placement(transformation(origin={-40,-40}, extent={{-10,-10},{10,10}})));
  Modelica.Fluid.Sources.MassFlowSource_h boundary2(nPorts=1, redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, m_flow=5, h=656775) 
    annotation (Placement(transformation(origin={-90,-20}, extent={{-10,-10},{10,10}})));
  Modelica.Fluid.Sources.MassFlowSource_h boundary3(nPorts=1, redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, m_flow=5, h=756775) 
