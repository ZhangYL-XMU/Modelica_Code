...
model compare
  ...
  TYThermoFluidSys.Pipelines.DistributePipe_Incompressible distributePipe_Incompressible(n=4, redeclare package Medium = SFR_Striling.Media.Sodium.Sodium, redeclare model DP = TYThermoFluidSys.Pipelines.Basic.DP_OnePhase.DarcyWeisbach, initOpt=TYThermoFluidSys.Utilities.Types.InitOptions.noInit, from_dp=false) 
    annotation (Placement(transformation(origin={-18,12},
extent={{-10,-10},{10,10}})));
  TYThermoFluidSys.Boundaries.BoundaryMdot boundaryMdot(m_flow=10, redeclare package Medium = TYMedia.Incompressible.Pb, energyDefinition="T", h=4.725e4,T=773.15) 
