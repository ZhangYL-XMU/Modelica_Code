...
//流阻模型    
  replaceable model Friction = BaseClasses.PressureDrop.Detail;
  Friction friction(
    redeclare package Medium = Medium,
    states = states,
    N = N,
...
//传热模型    
  replaceable model HeatTransfer = BaseClasses.HeatTransfer.circle
...
/************************************节点物性计算********************************************/
  for i in 1:N loop
    states[i] = Medium.setState_pTX(p[i], T[i]);
    h[i] = Medium.specificEnthalpy(states[i]);
  end for;
