within SFR_Striling.Test;
model medium_test
  Modelica.Fluid.Pipes.DynamicPipe pipe(redeclare package Medium = SFR_Striling.Media.Sodium.Sodium_table, length=10, diameter=0.1, T_start=573.15, nNodes=10, use_HeatTransfer=true, m_flow_start=100) 
    annotation (Placement(transformation(origin = {2.46492, -1.84869}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Fluid.Sources.MassFlowSource_h boundary(nPorts=1, redeclare package Medium = SFR_Striling.Media.Sodium.Sodium_table, m_flow=300) 
    annotation (Placement(transformation(origin = {-75.1802, -2.77304}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Fluid.Sources.Boundary_ph boundary1(nPorts=1, redeclare package Medium = SFR_Striling.Media.Sodium.Sodium_table) 
