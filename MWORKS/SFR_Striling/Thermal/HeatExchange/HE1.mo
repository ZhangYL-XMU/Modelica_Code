within SFR_Striling.Thermal.HeatExchange;
model HE1
...
points={{20,40},{-20,30},{20,20},{-20,10},{20,0},{-20,-10},{20,-20},{-20,-30},{20,-40}})}));
  ...
  TYThermoFluidSys.Thermal.TubeWall tubeWall(N=N - 1,
    ...
    extent={{-10,-10},{10,10}},
    rotation=90)));
  SFR_Striling.Fluid.Pipes.pipe shell(N=N, redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, L_total=L_total, Dh(displayUnit="mm")=Dh_shell, n_pipe=n_tubes, T_wall_start=T_wall_ref, m_flow_start=1, CF_HeatTransfer=CF_HeatTransfer, momentumDynamics=Modelica.Fluid.Types.Dynamics.SteadyState) 
    annotation (Placement(transformation(origin={7.10543e-15,-62},
    extent={{-10,-10},{10,10}})));
  SFR_Striling.Fluid.Pipes.pipe tube(redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium, N=N, L_total=L_total, n_pipe=n_tubes, Dh(displayUnit="mm")=Dh_tube, T_wall_start=T_wall_ref, m_flow_start=1, CF_HeatTransfer=CF_HeatTransfer, momentumDynamics=Modelica.Fluid.Types.Dynamics.SteadyState) 
  ...
  Modelica.Fluid.Interfaces.FluidPort_b shell_out(redeclare package Medium = Media.Sodium.ConstantPropertyLiquidSodium) 
    annotation (Placement(transformation(origin={102,-62.0181},
    extent={{-10,-10},{10,10}})));
  SFR_Striling.Thermal.HeatExchange.CounterFlowPair counterFlow(n=N - 1, counterCurrent=true) 
