...
model test_TubeWall
  ...
  Fluid.Pipes.pipe p1(N=2, n_pipe=1, L_total=1, Dh=0.0415, initFromEnthalpy=true, h_start=506655, wallHeatTransfer=true, momentumDynamics=Modelica.Fluid.Types.Dynamics.SteadyState, redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium) 
  ...
  Modelica.Fluid.Sources.MassFlowSource_T src(nPorts=1, T=703.15, m_flow=1, redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium) 
    annotation(Placement(transformation(origin={-120,0}, extent={{-10,-10},{10,10}})));
  Modelica.Fluid.Sources.Boundary_pT sink(nPorts=1, T=703.15, p=100000, redeclare package Medium = SFR_Striling.Media.Sodium.ConstantPropertyLiquidSodium) 
