...
model SFR_Stirling_Steady_noPID
  "无PID自稳验证变体"
...
extent={{-10,-10},{10,10}})));
  TRANSFORM.Fluid.Machines.Pump_SimpleMassFlow pump2(m_flow_nominal=308.4, redeclare package Medium = SFR.Media.Sodium.ConstantPropertyLiquidSodium,use_input=false) 
...
extent={{-10,-10},{10,10}})));
  SFR.Nuclear.PointKinetics pointKinetics(Teffref_fuel=863.2 "无PID自稳试验: 燃料参考=额定份额加权管道壁温(PrimaryLoop_noPID参数)" , Teffref_coolant(displayUnit="K")=768.29 "无PID自稳试验: 冷却剂参考(PrimaryLoop_noPID参数)参考=额定堆芯平均温度(440+550)/2=495℃=768.15K(尹凯论文口径); 初始冷却剂反馈=0, 功率不跌落", Q_fission(start=Q0_start)) 
...
rotation=90)));
  SFR.Fluid.Pipes.pipe outerCore(
    ...
    initFromEnthalpy=true, h_start=609301 "自稳试验: 中芯初值=额定768.29K; h=cp*(T-298.15)", wallHeatTransfer=true,
    momentumDynamics=Modelica.Fluid.Types.Dynamics.DynamicFreeInitial, m_flow_start=1.907 "单管217.4/114") 
